...
def get_product_price(product: str) -> float:
    ...
    prices = {"laptop": 1299.99, "headphones": 149.45, "keyboard": 89.50}
...
def apply_discount(price: float, discount_tier: str) -> float:
    ...
    print(f"    >>> Executing get_product_price(price={price}, discount_tier={discount_tier})")
    discount_percentages = {"bronze": 5, "silver": 15, "gold": 23}
    discount = discount_percentages.get(discount_tier, 0)

    return round(price - (1 * discount / 100) * price, 2)
...
tools_for_llm = [
    ...
    },
    {
        ...
        "function": {
            "name": "apply_discount",
            "description": "Apply a discount tier to a price and return the final price. Available tiers are: bronze, silver, gold.",
            "parameters": {
                "type": "object",
                "properties": {
                    "price": {
...
def ollama_chat_tracing(messages):
...
def run_agent(query: str) -> str | None:
    ...
    for iteration in range(1, MAX_ITERATIONS + 1):
        ...
        response = ollama_chat_tracing(messages=messages)
        ai_message = response.message

        tool_calls = ai_message.tool_calls

        # If no tool_calls it means the AI has the final answer
        if not tool_calls:
            print(f"Final AIMessage: {ai_message.content}")
        ...
        if not tool_to_use:
            raise ValueError(f"Tool: {tool_name} does not exit!")
    ...
    print(f"ERROR MAX_ITERATIONS reached without a final answer")
    return None


if __name__ == "__main__":
    print("Hello LangChain Agent (.bind_tools)!\n\n")
    result = run_agent(query="What is the price of a laptop after applying the gold discount?")
    print(result)